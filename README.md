...
| **证据与冲突仲裁** | 规范证据图谱（CEG）、确定性证据校准分公式、时间窗口作用域（`temporalScope`）、跨体系时空冲突检测器、Prompt Firewall 辩证防火墙、时域区间重叠与前驱关系推断（`inferEvidenceRelations`） | 跨子图图遍历剪枝算法 | 任意自然语言 Claim 的完全自动化逻辑形式化验证 |
