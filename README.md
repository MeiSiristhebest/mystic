...
| **四柱八字** | 全球 IANA 时区与真太阳时（EoT 均时差 + 经度时角差）、严格二十四节气换月换年（立春分界）、子初（23:00）换日、地支藏干（本/中/余气）、显性干支五行分布、月令十神信号、12 个边界 Golden Fixtures | 细粒度格局强弱旺衰定局模型 | 盲派口诀、铁板神数 |
| **吠陀占星** | IANA 国际时区与夏令时转换、JD 儒略日、Moshier 天文九曜真黄道、Fail-Fast 错误断言、Lahiri 线性岁差模型、纯 UTC 上升点 Lagna、27 月宿与 Pada、120 年全量 3 级递归 Vimshottari (9/81/729)、7-Chara Karakas、D1/D7/D9/D10/D12/D60 分盘映射 | 基础 Ashtakavarga 与星曜强弱定性 | C++ 原生 Swiss Ephemeris (`pysweph`) 绑定、JHora 桌面级 15 张全分盘精细运算 |
| **倪海厦中医** | 正向支持（Positive）、反向排除（Negative）、五大核心四诊缺失拒诊机制（`insufficient_evidence`）、条文与经典文献对应（具体版本与校勘依据见数据源）、八大金标准量化、先天五运六气大运客气 | 六经层级候选证型评分与排序 | 临床处方开具（系统仅提供文献候选证型与理法文化研究，严禁作为医疗处方） |
| **证据与冲突仲裁** | 规范证据图谱（CEG）、确定性证据校准分公式、分级证据类型与原典来源分级（`EvidenceType` / `SourceTier`）、时间窗口作用域（`temporalScope`）、跨体系时空冲突检测器、Prompt Firewall 辩证防火墙、时域区间重叠与语义前驱关系推断（`inferEvidenceRelations`） | 跨子图图遍历剪枝算法 | 任意自然语言 Claim 的完全自动化逻辑形式化验证 |
