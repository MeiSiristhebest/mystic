...
export default function FaceReadingApp({
  ...
  const handleGenerate = async () => {
    ...
    const prompt = `
      这是一次传统的东方${type === "face" ? "面相" : "手相"}分析。
      用户提供了一张${type === "face" ? "面部" : "手掌"}的照片。
      用户心中默念的问题或关注点是：“${sanitizedQuestion || "无具体问题，请全面分析"}”
      ${profileContext}

      请你作为一位精通中国传统相术的大师，仔细观察照片中的特征，并用中文提供一份专业、深刻、严谨的相学解读报告。
      ...
      （描述你观察到的主要${
        type === "face"
          ? "面部轮廓、五官特点（如三停五眼、十二宫等）"
          : "手型、主要掌纹（如生命线、智慧线、感情线等）及掌丘"
      }特征）
